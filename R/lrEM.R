...
if (rob==TRUE){
  ...
  while (iter_again == 1){
    ...
    for (npat in 2:length(levels(misspat))){                    
      if ((length(obs[[npat]]) == 1) & (!any(npat==alt.pat))){
        alt.in <- TRUE
        X[misspat==npat,] <- multRepl(X.old[misspat==npat,],label=NA,dl=dl,delta=delta)    
      ...
      if (length(obs[[npat]]) > 1) {
        ...
        for (m in 1:length(miss[[npat]])){
          p <- miss[[npat]][m]
          target <- X.old[,p]
          phi <- t(apply(cbind(dl=rep(dl[p],sum(misspat==npat)),feeder[misspat==npat,]),1,ilr))
  ...
  if (alt.in) {
    ...
    for (i in 2:length(alt.pat)){
      ...
      cat("   Affected samples: "); cat(alt.mr[[i]])
    }
  }
} # End robust lrEM
